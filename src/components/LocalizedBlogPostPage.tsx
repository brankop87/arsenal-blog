...
export default async function LocalizedBlogPostPage({ slug, locale }: Props) {
  ...
  const catLabel = getCategoryLabel(post.category, locale)
  const theme = getCategoryTheme(post.category)

  let dateFormatted = post.date
  try {
    dateFormatted = format(new Date(post.date), locale === 'de' ? 'd. MMMM yyyy' : 'MMMM d, yyyy', {
